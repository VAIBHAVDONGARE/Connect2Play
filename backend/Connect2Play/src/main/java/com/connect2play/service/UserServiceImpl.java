...
//    }
	...
	public boolean isEmailTaken(String email) {
		System.out.println("Checking email: " + email); // Debugging
        boolean exists = userRepository.existsByEmail(email);
        System.out.println("Email exists: " + exists); // Debugging
		...
		try {
			//modelMapper.map(updatedUserDTO, user);
			  user.setFullName(updatedUserDTO.getFullName());
		        ...
		        user.setPhNumber(updatedUserDTO.getPhNumber());
...
//	    }
//
//	    // Generate JWT token (Assuming JWT is being used)
...
//
//	    // Map User entity to UserDetailsResponseDTO
//	    UserDetailsResponseDTO userDetails = modelMapper.map(user, UserDetailsResponseDTO.class);
//
//	    return new AuthResponseDTO(accessToken, refreshToken, userDetails);
//	}
...
//	}
	...
	public void createTeam(User creator, TeamCreateDTO teamCreateDTO) {
		if (teamRepository.existsByTeamNameIgnoreCase(teamCreateDTO.getTeamName())) {
...
//	}
	...
	public List<ChallengeResponseDTO> getChallengesSentByUser(Long userId) {
		...
		Team userTeam = teamRepository.findByCreator(user);
		if (userTeam == null) {
